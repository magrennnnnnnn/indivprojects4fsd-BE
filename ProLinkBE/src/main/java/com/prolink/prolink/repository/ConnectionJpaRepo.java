...
public interface ConnectionJpaRepo extends JpaRepository<ConnectionEntity, Long> {
    ...
    Optional<ConnectionEntity> findByRequesterProfile_IdProfileAndReceiverProfile_IdProfile(
            Long requesterProfileId,
            Long receiverProfileId
    );

    List<ConnectionEntity> findByReceiverProfile_IdProfileAndStatusOrderByCreatedAtDesc(
            Long receiverProfileId,
            ConnectionStatusType status
    );

    List<ConnectionEntity> findByRequesterProfile_IdProfileAndStatusOrderByCreatedAtDesc(
            Long requesterProfileId,
            ConnectionStatusType status
    );

    List<ConnectionEntity> findByRequesterProfile_IdProfileOrReceiverProfile_IdProfile(
            Long requesterProfileId,
            Long receiverProfileId
    );
}
